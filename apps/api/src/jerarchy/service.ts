...
async function getJerarchies(filter: TGetJerarchyQueryDefinition = {}) {
    const jerarchies = await Jerarchy.find(filter).populate("employee").populate("superior");
    return jerarchies;
}

async function getJerarchy(_id: TGetJerarchyParamsDefinition["_id"]) {
    const jerarchy = await Jerarchy.findById(_id).populate("employee").populate("superior");
    return jerarchy;
...
async function getJerarchiesByEmployee(employeeId: string) {
    const jerarchies = await Jerarchy.find({ employee: employeeId }).populate("employee").populate("superior");
    return jerarchies;
}

async function getJerarchiesBySuperior(superiorId: string) {
    const jerarchies = await Jerarchy.find({ superior: superiorId }).populate("employee").populate("superior");
    return jerarchies;
}
